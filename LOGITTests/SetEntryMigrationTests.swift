...
final class SetEntryMigrationTests: XCTestCase {
    ...
    /// Count-checked access to a backfilled set's entries.
    ///
    /// `XCTAssertEqual` is a *soft* assertion: it records the failure and execution continues.
    /// A bare `XCTAssertEqual(set.entries.count, n)` followed by `set.entries[i]` therefore
    /// **traps** whenever the count is wrong — "Index out of range" takes down the whole test
    /// host, and with it every other result in the run, which reads as simulator flakiness
    /// rather than as the one assertion that failed. This records the same count failure, dumps
    /// the set's legacy fields (the input the backfill derived the entries from — the thing you
    /// actually need to know when the count is wrong), and hands back `nil` so callers skip
    /// their per-entry checks and the run continues.
    private func checkedEntries<Entry>(
        _ entries: [Entry],
        expected: Int,
        of set: NSManagedObject,
        _ message: String = "",
        ...
        guard entries.count == expected else {
            ...
            XCTFail(
                "Expected \(expected) entries, got \(entries.count)\(note). "
                    + "Legacy source: \(legacyDescription(of: set))",
                file: file, line: line
            )
            return nil
        }
    ...
    /// The legacy per-subclass fields a set's entries are derived from, for failure messages.
    private func legacyDescription(of set: NSManagedObject) -> String {
        func list(_ values: [Int64]?) -> String {
            values.map { "\($0)" } ?? "nil"
        }
        let id = (set.value(forKey: "id") as? UUID)?.uuidString ?? "nil"
        var description = "\(type(of: set))(id: \(id))"
        switch set {
        case let dropSet as DropSet:
            description += " repetitions: \(list(dropSet.repetitions)) weights: \(list(dropSet.weights))"
        case let templateDropSet as TemplateDropSet:
            description +=
                " repetitions: \(list(templateDropSet.repetitions)) weights: \(list(templateDropSet.weights))"
        case let standardSet as StandardSet:
            description += " repetitions: \(standardSet.repetitions) weight: \(standardSet.weight)"
        case let templateStandardSet as TemplateStandardSet:
            description +=
                " repetitions: \(templateStandardSet.repetitions) weight: \(templateStandardSet.weight)"
        case let superSet as SuperSet:
            description +=
                " first: \(superSet.repetitionsFirstExercise)x\(superSet.weightFirstExercise)"
                + " second: \(superSet.repetitionsSecondExercise)x\(superSet.weightSecondExercise)"
        case let templateSuperSet as TemplateSuperSet:
            description +=
                " first: \(templateSuperSet.repetitionsFirstExercise)x\(templateSuperSet.weightFirstExercise)"
                + " second: \(templateSuperSet.repetitionsSecondExercise)x\(templateSuperSet.weightSecondExercise)"
        default:
            break
        }
        return description
    ...
    func testBackfillCreatesEntriesForEveryLegacySet() throws {
        ...
        let standard1: StandardSet = try fetch("StandardSet", fixtureId: 21, in: context)
        if let entries = checkedEntries(standard1.entries, expected: 1, of: standard1) {
            assertEntry(entries[0], order: 0, repetitions: 12, weight: 80000, exercise: benchPress)
        }
        let standard2: StandardSet = try fetch("StandardSet", fixtureId: 22, in: context)
        if let entries = checkedEntries(standard2.entries, expected: 1, of: standard2) {
            assertEntry(entries[0], order: 0, repetitions: 0, weight: 0, exercise: benchPress)
        }

        // Drop sets → one entry per drop, malformed arrays padded, never truncated
        let drop1: DropSet = try fetch("DropSet", fixtureId: 31, in: context)
        if let entries = checkedEntries(drop1.entries, expected: 3, of: drop1) {
            assertEntry(entries[0], order: 0, repetitions: 10, weight: 140_000, exercise: squat)
            assertEntry(entries[1], order: 1, repetitions: 8, weight: 120_000, exercise: squat)
            assertEntry(entries[2], order: 2, repetitions: 6, weight: 100_000, exercise: squat)
        }
        let drop2: DropSet = try fetch("DropSet", fixtureId: 32, in: context)
        if let entries = checkedEntries(
            drop2.entries, expected: 2, of: drop2,
            "reps-longer drop set keeps every recorded value"
        ) {
            assertEntry(entries[0], order: 0, repetitions: 10, weight: 60000, exercise: squat)
            assertEntry(entries[1], order: 1, repetitions: 8, weight: 0, exercise: squat)
        }
        let drop3: DropSet = try fetch("DropSet", fixtureId: 33, in: context)
        if let entries = checkedEntries(
            drop3.entries, expected: 2, of: drop3,
            "weights-longer drop set keeps every recorded value"
        ) {
            assertEntry(entries[0], order: 0, repetitions: 12, weight: 50000, exercise: squat)
            assertEntry(entries[1], order: 1, repetitions: 0, weight: 40000, exercise: squat)
        }
        let drop4: DropSet = try fetch("DropSet", fixtureId: 34, in: context)
        if let entries = checkedEntries(
            drop4.entries, expected: 1, of: drop4,
            "nil-array drop set gets its placeholder entry"
        ) {
            assertEntry(entries[0], order: 0, repetitions: 0, weight: 0, exercise: squat)
        }
        let drop5: DropSet = try fetch("DropSet", fixtureId: 35, in: context)
        _ = checkedEntries(
            drop5.entries, expected: 1, of: drop5,
            "empty-array drop set gets its placeholder entry"
        )

        // Super sets → two entries, attributed to primary/secondary exercise
        let superSet1: SuperSet = try fetch("SuperSet", fixtureId: 41, in: context)
        if let entries = checkedEntries(superSet1.entries, expected: 2, of: superSet1) {
            assertEntry(entries[0], order: 0, repetitions: 10, weight: 80000, exercise: benchPress)
            assertEntry(entries[1], order: 1, repetitions: 12, weight: 25000, exercise: cableFly)
        }
        let superSet2: SuperSet = try fetch("SuperSet", fixtureId: 42, in: context)
        if let entries = checkedEntries(superSet2.entries, expected: 2, of: superSet2) {
            assertEntry(entries[0], order: 0, repetitions: 0, weight: 0, exercise: benchPress)
            assertEntry(entries[1], order: 1, repetitions: 0, weight: 0, exercise: cableFly)
        }
        // The group without a secondary exercise: second-slot values survive with no exercise
        let superSet3: SuperSet = try fetch("SuperSet", fixtureId: 43, in: context)
        if let entries = checkedEntries(superSet3.entries, expected: 2, of: superSet3) {
            assertEntry(entries[0], order: 0, repetitions: 8, weight: 30000, exercise: cableFly)
            assertEntry(entries[1], order: 1, repetitions: 15, weight: 10000, exercise: nil)
        }

        // Orphan sets: entries exist, no exercise attribution
        let orphanStandard: StandardSet = try fetch("StandardSet", fixtureId: 51, in: context)
        if let entries = checkedEntries(orphanStandard.entries, expected: 1, of: orphanStandard) {
            assertEntry(entries[0], order: 0, repetitions: 5, weight: 200_000, exercise: nil)
        }
        let orphanDrop: DropSet = try fetch("DropSet", fixtureId: 52, in: context)
        if let entries = checkedEntries(orphanDrop.entries, expected: 2, of: orphanDrop) {
            assertEntry(entries[0], order: 0, repetitions: 3, weight: 180_000, exercise: nil)
            assertEntry(entries[1], order: 1, repetitions: 2, weight: 190_000, exercise: nil)
        }

        // Template mirrors
        let templateStandard: TemplateStandardSet =
            try fetch("TemplateStandardSet", fixtureId: 71, in: context)
        if let entries = checkedEntries(templateStandard.entries, expected: 1, of: templateStandard) {
            XCTAssertEqual(entries[0].repetitions, 10)
            XCTAssertEqual(entries[0].weight, 77500)
            XCTAssertEqual(entries[0].exercise, benchPress)
            XCTAssertEqual(entries[0].type, .repsAndWeight)
        }
        let templateDrop: TemplateDropSet =
            try fetch("TemplateDropSet", fixtureId: 72, in: context)
        if let entries = checkedEntries(templateDrop.entries, expected: 2, of: templateDrop) {
            XCTAssertEqual(entries[0].repetitions, 9)
            XCTAssertEqual(entries[0].weight, 130_000)
            XCTAssertEqual(entries[1].repetitions, 7)
            XCTAssertEqual(entries[1].weight, 0)
        }
        let templateSuper: TemplateSuperSet =
            try fetch("TemplateSuperSet", fixtureId: 73, in: context)
        if let entries = checkedEntries(templateSuper.entries, expected: 2, of: templateSuper) {
            XCTAssertEqual(entries[0].repetitions, 8)
            XCTAssertEqual(entries[0].weight, 70000)
            XCTAssertEqual(entries[0].exercise, benchPress)
            XCTAssertEqual(entries[1].repetitions, 10)
            XCTAssertEqual(entries[1].weight, 20000)
            XCTAssertEqual(entries[1].exercise, cableFly)
        }
    ...
    func testBackfillIsIdempotent() throws {
        ...
        XCTAssertEqual(entityCount("TemplateSetEntry", in: context), 5)
        let drop1: DropSet = try fetch("DropSet", fixtureId: 31, in: context)
        _ = checkedEntries(drop1.entries, expected: 3, of: drop1)
    }
    ...
    func testBackfillPicksUpLateArrivingLegacySets() throws {
        ...
        if let entries = checkedEntries(lateSet.entries, expected: 1, of: lateSet) {
            XCTAssertEqual(entries[0].repetitions, 9)
            XCTAssertEqual(entries[0].weight, 55000)
        }
    }
    ...
    func testBackfillDoesNotTouchSetsThatAlreadyHaveEntries() throws {
        ...
        if let entries = checkedEntries(newFormatSet.entries, expected: 1, of: newFormatSet) {
            XCTAssertEqual(entries[0].repetitions, 99)
            XCTAssertEqual(entries[0].weight, 999_000)
        }
