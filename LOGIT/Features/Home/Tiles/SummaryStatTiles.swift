...
/// frequency divided out), with a strip of that same window's bins underneath. The "per workout"
/// subtitle names the basis right under the number. Parallel to `WorkoutStatTile` (which reads one
/// session), but reading the Summary's one timeframe instead.
///
...
/// on the tile honours that — the number and every bar are the selected window and nothing else, so
/// reading "10,000 kg" off one tile and "+3.5%" off the Strength tile beside it are statements about
